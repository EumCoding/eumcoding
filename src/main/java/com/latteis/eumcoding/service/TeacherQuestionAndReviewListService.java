...
public class TeacherQuestionAndReviewListService {
     ...
     * 질문 글들의 모음
     */
    public List<QuestionDTO.StudentQuestionListDTO> getMyStudentQuestions(int memberId, LocalDate start, LocalDate end,Integer lectureId, int page, int size) {
        ...
        List<QuestionDTO.StudentQuestionListDTO> studentQuestionListDTOS = questionsPage.getContent().stream().map(question -> {
            // 해당 질문에 대한 답변 유무 확인
            ...
            return QuestionDTO.StudentQuestionListDTO.builder()
        ...
        return studentQuestionListDTOS;
    }
